...
SPOKES=$(bart show -d 2 ${DATA})
COILS=$(bart show -d 3 ${DATA})
FRAMES=$(bart show -d 5 ${DATA})
BR=$(($(bart show -d 1 ${DATA})/2))
SAMPLES=$(echo $BR 2 | awk '{printf "%3.0f\n",$1*$2}')

TIME_STEPS=$FRAMES   # Integer division!
...
Bloch_short)
        ...
        then
                ...
                echo "Please run b1 script first."
                exit 1
        fi
...
Bloch_short_hp)
	...
	NOM_SLICE_THICKNESS=0.001
        ...
        nice -5 bart moba --bloch --sim STM \
        --img_dims $BR:$BR:1 \
        --seq IR-BSSFP,TR=${TR},TE=${TE},FA=${FA},Trf=${RF_DUR},BWTP=${BWTP},pinv,ipl=${INV_LEN},ppl=${PREP_LEN},sl-grad=${SS_GRAD_STRENGTH},isp=${INV_SPOILER},av-spokes=${AV},slice-thickness=${SLICE_THICKNESS},nom-slice-thickness=${NOM_SLICE_THICKNESS},Nspins=${SLICE_PROFILE_SPINS} \
	--other pinit=1:1:1:0,pscale=1:1:30:0,b1map=${B1MAP},b1-sobolev-a=440,b1-sobolev-b=22 --scale_data=5000. --scale_psf=1000. --normalize_scaling \
        -i$ITER -C$INNER_ITER -s$STEP_SIZE -B$MIN_R1 -d4 -o$OS -R$REDU_FAC -j$LAMBDA -g -N \
	--pusteps 4 --ratio 0.5 \
        -t ${TRAJ} ${DATA} TI reco sens \
...
Bloch_short_b0)
	...
	if [ ! -f ${B0MAP}".cfl" ];
        then
                echo "B0 map $B0MAP does not exist!"
                echo "Please run b0 script first."
                exit 1
        fi
        ...
        --img_dims $BR:$BR:1 \
        --seq IR-BSSFP,TR=${TR},TE=${TE},FA=${FA},Trf=${RF_DUR},BWTP=${BWTP},ipl=${INV_LEN},ppl=${PREP_LEN},sl-grad=${SS_GRAD_STRENGTH},isp=${INV_SPOILER},av-spokes=${AV},slice-thickness=${SLICE_THICKNESS},nom-slice-thickness=${NOM_SLICE_THICKNESS},Nspins=${SLICE_PROFILE_SPINS} \
	--other pinit=1:1:1:0,pscale=1:1:30:0,b0map=${B0MAP},b1map=${B1MAP},b1-sobolev-a=440,b1-sobolev-b=22 --scale_data=5000. --scale_psf=1000. --normalize_scaling \
        -i$ITER -C$INNER_ITER -s$STEP_SIZE -B$MIN_R1 -d4 -o$OS -R$REDU_FAC -j$LAMBDA -g -N \
	--pusteps 4 --ratio 0.5 \
        -t ${TRAJ} ${DATA} TI reco sens \
        2>&1 | tee reco.log
